...
async function carregarMultas() {
    ...
    tbody.innerHTML = fines.map(m => {
        ...
        const notifStatus = m.notificada ? `<span class="badge-custom badge-success" title="Ofício: ${m.numero_oficio || '-'}"><i class="bi bi-check-circle"></i> Sim</span>` : '<span class="badge-custom badge-warning"><i class="bi bi-x-circle"></i> Não</span>';
        return `<tr><td><strong>${m.numero_ait || '-'}</strong></td><td>${m.tipo || '-'}</td><td>${m.placa || '-'}</td><td>${m.secretaria || '-'}</td><td>${m.condutor_infrator || '-'}</td><td>${formatDate(m.data_hora_infracao)}</td><td>R$ ${formatCurrency(m.valor)}</td><td>${getIndicacaoCondutorStatus(m)}</td><td>${notifStatus}</td><td>${pr !== null ? `<span class="prazo-indicator ${getPrazoClass(pr)}">${pr}d</span>` : '-'}</td><td>${getStatusBadge(m.status)}</td><td><button class="action-btn" onclick="verMulta('${m.id}')"><i class="bi bi-eye"></i></button><button class="action-btn" onclick="editarMulta('${m.id}')"><i class="bi bi-pencil"></i></button><button class="action-btn danger" onclick="excluirMulta('${m.id}')"><i class="bi bi-trash"></i></button></td></tr>`;
    }).join('');
}
...
async function salvarMulta() {
    ...
    try {
        ...
        const data = {
            ...
            notificada: document.getElementById('multaNotificada').checked,
            numero_oficio: document.getElementById('multaNumeroOficio').value.trim() || null,
            data_oficio: document.getElementById('multaDataOficio').value || null,
            protocolo_oficio: document.getElementById('multaProtocoloOficio').value.trim() || null
        };
...
function toggleCamposNotificacao() {
    const checked = document.getElementById('multaNotificada').checked;
    document.getElementById('camposNotificacao').classList.toggle('hidden', !checked);
}
...
async function verMulta(id) {
    ...
    // Notificação à Secretaria
    html += `<div class="mt-3 p-3" style="background:var(--bg-tertiary);border-radius:8px;border:1px solid var(--border-color);"><h6 style="color:var(--text-muted);margin-bottom:0.75rem;font-size:0.85rem;"><i class="bi bi-envelope-paper me-2"></i>NOTIFICAÇÃO À SECRETARIA</h6>`;
    if (m.notificada) {
        html += `<table class="table-custom"><tr><td style="width:40%;"><strong>Status</strong></td><td><span class="badge-custom badge-success">Notificada</span></td></tr><tr><td><strong>Nº Ofício</strong></td><td>${m.numero_oficio || '-'}</td></tr><tr><td><strong>Data Ofício</strong></td><td>${formatDate(m.data_oficio)}</td></tr><tr><td><strong>Protocolo</strong></td><td>${m.protocolo_oficio || '-'}</td></tr></table>`;
    } else {
        html += `<p style="color:var(--text-muted);"><i class="bi bi-x-circle me-1"></i> Secretaria ainda não foi notificada</p>`;
    }
    html += `</div>`;
...
async function editarMulta(id) {
    ...
    // Carregar select de condutores e selecionar o atual
    await carregarCondutoresSelect();
    document.getElementById('multaCondutorSelect').value = m.condutor_infrator || '';
    // Campos de notificação
    document.getElementById('multaNotificada').checked = m.notificada === true;
    document.getElementById('multaNumeroOficio').value = m.numero_oficio || '';
    document.getElementById('multaDataOficio').value = m.data_oficio ? m.data_oficio.slice(0, 10) : '';
    document.getElementById('multaProtocoloOficio').value = m.protocolo_oficio || '';
    toggleCamposNotificacao();
...
function limparFormularioMulta() {
    ...
    document.getElementById('multaCondutor').value = '';
    document.getElementById('multaCondutorIndicado').checked = false;
    document.getElementById('multaNotificada').checked = false;
    document.getElementById('multaNumeroOficio').value = '';
    document.getElementById('multaDataOficio').value = '';
    document.getElementById('multaProtocoloOficio').value = '';
    document.getElementById('camposNotificacao').classList.add('hidden');
...
async function gerarRelatorios() {
    ...
    const secretarias = new Set(), orgaos = new Set();
    fines.forEach(m => { if (m.secretaria) secretarias.add(m.secretaria); if (m.orgao_autuador) orgaos.add(m.orgao_autuador); });
    const selSec = document.getElementById('relSecretaria');
    const selOrg = document.getElementById('relOrgao');
    if (selSec.options.length <= 1) { Array.from(secretarias).sort().forEach(s => { selSec.innerHTML += `<option value="${s}">${s}</option>`; }); }
    if (selOrg.options.length <= 1) { Array.from(orgaos).sort().forEach(o => { selOrg.innerHTML += `<option value="${o}">${o}</option>`; }); }
    
    // Obter filtros
    const filtroDataInicio = document.getElementById('relDataInicio').value;
    const filtroDataFim = document.getElementById('relDataFim').value;
    const filtroTipo = document.getElementById('relTipo').value;
    const filtroSecretaria = document.getElementById('relSecretaria').value;
    const filtroVeiculo = document.getElementById('relVeiculo').value.toUpperCase().trim();
    const filtroOrgao = document.getElementById('relOrgao').value;
    const filtroStatus = document.getElementById('relStatus').value;
    
    // Aplicar filtros
    let filtradas = fines.filter(m => {
        if (filtroTipo && m.tipo !== filtroTipo) return false;
        if (filtroSecretaria && m.secretaria !== filtroSecretaria) return false;
        if (filtroVeiculo && (!m.placa || !m.placa.toUpperCase().includes(filtroVeiculo))) return false;
        if (filtroOrgao && m.orgao_autuador !== filtroOrgao) return false;
        if (filtroStatus && m.status !== filtroStatus) return false;
        if (filtroDataInicio && m.data_hora_infracao && m.data_hora_infracao < filtroDataInicio) return false;
        if (filtroDataFim && m.data_hora_infracao && m.data_hora_infracao > filtroDataFim) return false;
        return true;
    });
    
    // Mostrar resumo se há filtros ativos
    const temFiltro = filtroTipo || filtroSecretaria || filtroVeiculo || filtroOrgao || filtroStatus || filtroDataInicio || filtroDataFim;
    document.getElementById('resumoFiltrado').style.display = temFiltro ? 'block' : 'none';
    
    // Calcular estatísticas
    const totalMultas = filtradas.length;
    const valorTotal = filtradas.reduce((s, m) => s + (parseFloat(m.valor) || 0), 0);
    const emAberto = filtradas.filter(m => m.status === 'EM_ABERTO' || m.status === 'RECURSO_EM_JULGAMENTO').length;
    const pagas = filtradas.filter(m => m.status === 'PAGO').length;
    
    document.getElementById('relTotalMultas').textContent = totalMultas;
    document.getElementById('relValorTotal').textContent = 'R$ ' + formatCurrency(valorTotal);
    document.getElementById('relEmAberto').textContent = emAberto;
    document.getElementById('relPagas').textContent = pagas;
    document.getElementById('relContagem').textContent = totalMultas + ' registros';
    
    // Renderizar tabela detalhada
    const tbody = document.getElementById('tbodyRelatorio');
    if (!filtradas.length) {
        tbody.innerHTML = '<tr><td colspan="8"><div class="empty-state"><i class="bi bi-inbox" style="font-size:2rem;color:var(--text-muted);"></i><p>Nenhuma multa encontrada</p></div></td></tr>';
    } else {
        tbody.innerHTML = filtradas.map(m => `<tr>
            <td><strong>${m.numero_ait || '-'}</strong></td>
            <td>${m.tipo || '-'}</td>
            <td>${m.placa || '-'}</td>
            <td>${m.secretaria || '-'}</td>
            <td>${m.orgao_autuador || '-'}</td>
    ...
    // Rankings (usando dados filtrados)
    ...
    window.dadosRelatorio = { filtradas, secRank, condRank, totalMultas, valorTotal, emAberto, pagas, filtros: { tipo: filtroTipo, secretaria: filtroSecretaria, veiculo: filtroVeiculo, orgao: filtroOrgao, status: filtroStatus, dataInicio: filtroDataInicio, dataFim: filtroDataFim } };
}

function imprimirRelatorio() {
    const dados = window.dadosRelatorio;
    if (!dados || !dados.filtradas.length) { showToast('Nenhum dado para imprimir. Aplique filtros primeiro.', 'warning'); return; }
    
    const filtrosTexto = [];
    if (dados.filtros.tipo) filtrosTexto.push('Tipo: ' + dados.filtros.tipo);
    if (dados.filtros.secretaria) filtrosTexto.push('Secretaria: ' + dados.filtros.secretaria);
    if (dados.filtros.veiculo) filtrosTexto.push('Veículo: ' + dados.filtros.veiculo);
    if (dados.filtros.orgao) filtrosTexto.push('Órgão: ' + dados.filtros.orgao);
    if (dados.filtros.status) filtrosTexto.push('Status: ' + dados.filtros.status);
    if (dados.filtros.dataInicio) filtrosTexto.push('De: ' + formatDate(dados.filtros.dataInicio));
    if (dados.filtros.dataFim) filtrosTexto.push('Até: ' + formatDate(dados.filtros.dataFim));
    
    // Colunas dinâmicas - omitir coluna se filtro ativo
    const mostrarSecretaria = !dados.filtros.secretaria;
    const mostrarOrgao = !dados.filtros.orgao;
    const mostrarTipo = !dados.filtros.tipo;
    
    let headerCols = '<th>AIT</th>';
    if (mostrarTipo) headerCols += '<th>Tipo</th>';
    headerCols += '<th>Placa</th>';
    if (mostrarSecretaria) headerCols += '<th>Secretaria</th>';
    headerCols += '<th>Condutor</th>';
    if (mostrarOrgao) headerCols += '<th>Órgão</th>';
    headerCols += '<th>Data</th><th>Valor</th><th>Status</th>';
    
    const bodyRows = dados.filtradas.map(m => {
        let row = `<td>${m.numero_ait || '-'}</td>`;
        if (mostrarTipo) row += `<td>${m.tipo || '-'}</td>`;
        row += `<td>${m.placa || '-'}</td>`;
        if (mostrarSecretaria) row += `<td>${m.secretaria || '-'}</td>`;
        row += `<td>${m.condutor_infrator || '-'}</td>`;
        if (mostrarOrgao) row += `<td>${m.orgao_autuador || '-'}</td>`;
        row += `<td>${formatDate(m.data_hora_infracao)}</td>`;
        row += `<td>R$ ${formatCurrency(m.valor)}</td>`;
        row += `<td>${m.status ? m.status.replace(/_/g, ' ') : '-'}</td>`;
        return `<tr>${row}</tr>`;
    }).join('');
    
    const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Relatório SGMV</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .filtros strong { display: block; margin-bottom: 5px; }
        .resumo { display: flex; justify-content: space-around; margin-bottom: 20px; }
        .resumo-item { text-align: center; padding: 10px; background: #f9f9f9; border-radius: 4px; min-width: 100px; }
        .resumo-item .valor { font-size: 16px; font-weight: bold; color: #333; }
    ...
    <div class="header">
        <h1>SGMV - Sistema de Gestão de Multas Veiculares</h1>
        <p>Relatório gerado em ${new Date().toLocaleString('pt-BR')}</p>
    </div>
    ${filtrosTexto.length ? `<div class="filtros"><strong>Filtros aplicados:</strong> ${filtrosTexto.join(' | ')}</div>` : ''}
    <div class="resumo">
        <div class="resumo-item"><div class="valor">${dados.totalMultas}</div><div class="label">Total de Multas</div></div>
        <div class="resumo-item"><div class="valor">R$ ${formatCurrency(dados.valorTotal)}</div><div class="label">Valor Total</div></div>
        <div class="resumo-item"><div class="valor">${dados.emAberto}</div><div class="label">Em Aberto</div></div>
        <div class="resumo-item"><div class="valor">${dados.pagas}</div><div class="label">Pagas</div></div>
    </div>
    <table>
        <thead><tr>${headerCols}</tr></thead>
        <tbody>${bodyRows}</tbody>
    </table>
    <div class="footer">SGMV v3 - Documento gerado automaticamente</div>
...
// Carregar condutores no select da multa
async function carregarCondutoresSelect() {
    const select = document.getElementById('multaCondutorSelect');
    if (!select) return;
    const condutores = await DB.getCondutores();
    const valorAtual = document.getElementById('multaCondutor').value;
    select.innerHTML = '<option value="">Selecione ou cadastre novo...</option>' + 
        condutores.map(c => `<option value="${c.nome}" ${c.nome === valorAtual ? 'selected' : ''}>${c.nome}${c.secretaria ? ' (' + c.secretaria + ')' : ''}</option>`).join('');
}

function selecionarCondutor() {
    const select = document.getElementById('multaCondutorSelect');
    document.getElementById('multaCondutor').value = select.value;
}

let voltarParaMultaAposCadastro = false;
function abrirCadastroCondutorRapido() {
    voltarParaMultaAposCadastro = true;
    document.getElementById('condutorId').value = '';
    document.getElementById('formCondutor').reset();
    // Preencher secretaria com a da multa atual se disponível
    const secMulta = document.getElementById('multaSecretaria').value;
    if (secMulta) document.getElementById('condutorSecretaria').value = secMulta;
    document.getElementById('modalCondutorTitle').textContent = 'Cadastrar Condutor';
...
async function salvarCondutor() {
    ...
    try { 
        if (id) { await DB.updateCondutor(id, data); showToast('Atualizado!', 'success'); } 
        else { await DB.addCondutor(data); showToast('Cadastrado!', 'success'); } 
        hideModal('modalCondutor'); 
        carregarCondutores();
        // Se veio do cadastro rápido na multa, atualizar select e selecionar o novo condutor
        if (voltarParaMultaAposCadastro) {
            voltarParaMultaAposCadastro = false;
            await carregarCondutoresSelect();
            document.getElementById('multaCondutor').value = nome;
            document.getElementById('multaCondutorSelect').value = nome;
...
window.confirmarImportacao = confirmarImportacao; window.gerarRelatorios = gerarRelatorios; window.imprimirRelatorio = imprimirRelatorio; window.toggleCamposNotificacao = toggleCamposNotificacao;
...
window.abrirNovoCondutor = abrirNovoCondutor; window.salvarCondutor = salvarCondutor; window.abrirCadastroCondutorRapido = abrirCadastroCondutorRapido; window.selecionarCondutor = selecionarCondutor;
