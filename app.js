...
function imprimirRelatorio() {
    ...
    <div class="header">
        ...
        <p>AMTC - Autarquia Municipal de Transporte Coletivo - Rondonópolis/MT</p>
    ...
    <table>
        <thead><tr><th>AIT</th><th>Tipo</th><th>Placa</th><th>Secretaria</th><th>Órgão</th><th>Data</th><th>Valor</th><th>Status</th></tr></thead>
        <tbody>${dados.filtradas.map(m => `<tr>
            <td>${m.numero_ait || '-'}</td>
            <td>${m.tipo || '-'}</td>
            <td>${m.placa || '-'}</td>
            <td>${m.secretaria || '-'}</td>
            ...
            <td>${formatDate(m.data_hora_infracao)}</td>
            <td>R$ ${formatCurrency(m.valor)}</td>
            <td>${m.status ? m.status.replace(/_/g, ' ') : '-'}</td>
        </tr>`).join('')}</tbody>
    </table>
